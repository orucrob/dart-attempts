...
class MyModel extends Object with ChangeNotifier{
  @reflectable @observable String get foo => __$foo; String __$foo; @reflectable set foo(String value) { __$foo = notifyPropertyChange(#foo, __$foo, value); }
  String toString(){
...
class MyList extends PolymerElement {
  ...
  void load(){
    items.clear();
    store.allByIndex("name", only:"prvy").listen((map){
...
class IndexedDbStore<V> extends Store<V> {
  ...
  Stream<V> _allByIndex(String idxName,{Object only, Object lower, Object upper, bool lowerOpen:false, bool upperOpen:false} ) {
    ...
    if(only!=null){
     range = idb.KeyRange.only_(only);
    }else if(lower!=null && upper!=null){
      range = idb.KeyRange.bound_(lower, upper, lowerOpen, upperOpen );
    }else if(lower!=null ){
      range = idb.KeyRange.lowerBound_(lower, lowerOpen);
    }else if(upper!=null){
      range = idb.KeyRange.upperBound_(upper, upperOpen );
    }
